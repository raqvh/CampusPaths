...
public final class RatPoly {
  ...
  */
  private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
    for(int i = 0; i < lst.size(); i++) {
      ...
      RatTerm scal = new RatTerm(scalar, 0);
      lst.set(i, temp.mul(scal));
    }
  }
  ...
  */
  public RatPoly negate() {
    List<RatTerm> rt = new ArrayList<RatTerm>();
    for(RatTerm term : this.terms) {
      rt.add(term.negate());
    }
    RatPoly newRP = new RatPoly(rt);
    return newRP;
  }
  ...
  */
  public RatPoly mul(RatPoly p) {
    ...
    List<RatTerm> rt = new ArrayList<RatTerm>();
    for(RatTerm term : this.terms) {
      for(RatTerm mulTerm : p.terms) {
        sortedInsert(rt, term.mul(mulTerm));
      }
    }
    return new RatPoly(rt);
  }
  ...
  */
  public RatPoly div(RatPoly p) {
    ...
    //this / p = quotient
    List<RatTerm> quotient = new ArrayList<RatTerm>();
    RatPoly left = new RatPoly(this.terms);
    
    if(!this.equals(ZERO)) {
      while(p.degree() <= left.degree() && left.terms.size() != 0) {
        RatTerm d = left.getTerm(left.degree()).div(p.getTerm(p.degree()));
        sortedInsert(quotient, d);
        //then for each term in p we want to multiply by d
        //after this subtract
        left = left.sub(p.mul(new RatPoly(d)));
      }
    }
  ...
  */
  public RatPoly differentiate() {
    ...
    if(!rt.isEmpty()) {
      if(rt.get(rt.size()-1).getExpt() == 0) {
        rt.remove(rt.size()-1);
      }
    }
    for(int i = 0; i < rt.size(); i++) {
      rt.set(i, rt.get(i).differentiate());
    }
    return new RatPoly(rt);
  }
  ...
  */
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
    ...
    for(int i = 0; i < rt.size(); i++) {
      rt.set(i, rt.get(i).antiDifferentiate());
    }
    RatTerm constant = new RatTerm(integrationConstant, 0);
    sortedInsert(rt, constant);
    return new RatPoly(rt);
  }
  ...
  */
  public double integrate(double lowerBound, double upperBound) {
    ...
    RatPoly integrated = this.antiDifferentiate(new RatNum(0));
    return (integrated.eval(upperBound) - integrated.eval(lowerBound));
  }
